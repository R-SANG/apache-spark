...
class Performance():
    ...
    def originated_airport_with_least_flights(self, out_dir):
        orig_airports = self.df.groupBy('Origin').count().orderBy(('count'))
        self.sc.parallelize(orig_airports.collect()).saveAsTextFile(out_dir)
        return orig_airports.first()
    
    def destination_airport_with_least_flights(self, out_dir):
        orig_airports = self.df.groupBy('Dest').count().orderBy(('count'))
        self.sc.parallelize(orig_airports.collect()).saveAsTextFile(out_dir)
        return orig_airports.first()
        
    def destination_airport_with_most_flights(self, out_dir):
        orig_airports = self.df.groupBy('Dest').count().orderBy(desc('count'))
        self.sc.parallelize(orig_airports.collect()).saveAsTextFile(out_dir)
        return orig_airports.first()
...
def main(argv):
    ...
    print('{} has the most destination flights at {}'.format(least_count['Dest'], most_count['count']))
